...
final class MegaMenuWalker extends Walker_Nav_Menu
{
    ...
    public function start_lvl(&$output, $depth = 0, $args = [])
    {
        ...
        if ($this->has_children && 'yes' !== $this->current_item_settings['is_mega']) {
    ...
    public function start_el(&$output, $item, $depth = 0, $args = [], $id = 0)
    {
        ...
        $html_classes = ($content && ('yes' === $settings['is_mega'])) ? $html_classes . ' menu-item-has-children cmm4e-item-has-content' . $class_hide_sub : $html_classes;
	    ...
	    if (($this->has_children || 'yes' === $settings['is_mega']) && !$this->is_mobile) {
    ...
    private function isViewable($viewers)
    {
        ...
        if ($user->exists()) {
            ...
            $user_minimal_role = 'anyone';
        }

        if (in_array($user_minimal_role, $valid_roles) || in_array('anyone', $viewers)) {
            return true;
        }
